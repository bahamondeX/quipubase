...
if not PROJECT_ID:
    ...
    sys.exit(1) # Exit if essential env var is missing
...
class SpeechCreateParams(BaseModel):
    ...
    # EXPANDED: Added new voice names for multilingual tests
    voice: Literal[
        "alloy", "echo", "fable", "onyx", "nova", "shimmer",
        "es-es-standard-a", "fr-fr-wavenet-b", "de-de-neural2-c", "ja-jp-neural2-a"
    ] = Field(
        ...,
        description=(
...
class GoogleTTSService:
    ...
    """
    # EXPANDED: Added multilingual voice mappings
    _voice_mapping: Dict[str, Dict[str, str]] = {
        # English voices (Neural2 are high quality)
        ...
        "es-es-standard-a": {"language_code": "es-ES", "name": "es-ES-Standard-A"}, # Female
        # French (France) voices
        "fr-fr-wavenet-b": {"language_code": "fr-FR", "name": "fr-FR-Wavenet-B"},    # Female
        # German (Germany) voices
        "de-de-neural2-c": {"language_code": "de-DE", "name": "de-DE-Neural2-C"},    # Female
        # Japanese (Japan) voices
        "ja-jp-neural2-a": {"language_code": "ja-JP", "name": "ja-JP-Neural2-A"},    # Female
    }

    _audio_encoding_mapping: Dict[str, texttospeech.AudioEncoding] = {
        "mp3": texttospeech.AudioEncoding.MP3,
        "opus": texttospeech.AudioEncoding.OGG_OPUS,
        "aac": texttospeech.AudioEncoding.MP3, # Google recommends MP3 for AAC-like output
    ...
    def create_speech(self, params: SpeechCreateParams) -> bytes:
        """
        ...
        if not google_voice_config:
            raise ValueError(f"Unsupported voice: '{params.voice}'. No Google Cloud mapping found.")
        ...
        audio_encoding = self._audio_encoding_mapping.get(params.response_format or "mp3") # Default to mp3
