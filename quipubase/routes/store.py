from ..store import VectorStore, EmbeddingModel, UpsertResponse, DeleteResponse, Embedding, QueryResponse, QueryMatch
...
def store_router()->APIRouter:
	...
	async def _(
		...
		try:
			...
			embeddings = [Embedding(content=text, embedding=store.embed(text)) for text in data.content]
	...
	async def _(
		...
		"""
		try:
			store = VectorStore(namespace=data.namespace)
