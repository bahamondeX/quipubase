    ...
    def extract(self) -> tp.Generator[str, None, None]:
        ...
        try:
            ...
            if hasattr(doc.core_properties, "author") and doc.core_properties.author:
                prop_html.append(f"<p><strong>Author:</strong> {doc.core_properties.author}</p>")
            ...
            for paragraph in doc.paragraphs:
                if paragraph.text:
                    ...
                    if style_name and style_name.startswith("Heading"):
                        heading_level = style_name.split(' ')[-1] if ' ' in style_name else style_name[-1]
                        try:
                            ...
                            logger.error(f"Invalid heading level: {heading_level}. Skipping paragraph.")
                                        ...
                                        if pic.tag.endswith("blip"):  # type: ignore
                                            try:
                                                ...
                                                if image_id:
                                                    ...
                                                    img_data = base64.b64encode(image_part.blob).decode()
                                                    # Try to determine image type from the part content type
                                                    content_type = image_part.content_type
                                                    img_type = content_type.split("/")[-1] if "/" in content_type else "png"
