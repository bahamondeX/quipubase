...
class Collection(BaseModel):
    ...
    def create(self) -> None:
        ...
        _id = self.id
        assert isinstance(_id, UUID), "ID was not autogenerated"
        data = self.model_dump_json().encode("utf-8")
        self.col().put(_id.bytes, data)  # pylint: disable=E1101
        assert (
            self.col().get(_id.bytes) == data  # pylint: disable=E1101
        ), f"Failed to persist record {_id}"
