...
def route():
    ...
    async def _(
        file: UploadFile,
        model: tp.Literal["whisper-large-v3", "whisper-large-v3-turbo"] = Form(
            ...,
            description="The name of the model to use for transcription. Currently ignored as Google Cloud Speech-to-Text handles model selection internally or via configuration. OpenAI models like 'whisper-1' are for compatibility.",
        ),
        size: str = Form(
            "auto",
            description="The size of the model to use for transcription. Currently ignored as Google Cloud Speech-to-Text handles model selection internally.",
        ),
        response_format: tp.Literal["text", "json", "verbose_json"] = Form(
            "verbose_json",
            description="The format of the transcript output. Only 'json' is fully supported, 'text' will return plain text. Other OpenAI formats like 'srt', 'vtt', 'verbose_json' are not directly supported and will default to 'json' behavior.",
        ),
        temperature: float = Form(
            0.0,
            description="The sampling temperature, between 0 and 1. Higher values like 0.8 will make the output more random, while lower values like 0.2 will make it more focused and deterministic. Not directly used by Google Cloud Speech-to-Text.",
        ),
        prompt: Optional[str] = Form(
            None,
            description="An optional text to guide the model's style or continue a previous audio segment. Not directly used by Google Cloud Speech-to-Text.",
        ),
    ):
        """
        ...
        content_type = file.content_type.split("/")[1].split(";")[
            0
        ]  # Extract the main type (e.g., "mp3", "wav", etc.)
