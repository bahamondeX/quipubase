...
class AnyDocs:
    ...
    def from_module(cls, module_name: str) -> None:
        ...
        try:
            ...
            if not loaders_found:
                raise ImportError(
                    f"No valid loader classes found in module {module_name}"
                )

        except (ImportError, AttributeError) as e:
            raise ImportError(
                f"Could not import loaders from module {module_name}: {str(e)}"
            )
    ...
    @classmethod
    def load_document(
        cls, file: tp.Union[str, Path, UploadFile], extract_text_only: bool = True
        ...
        if isinstance(file, str):
            if file.startswith(("http://", "https://")):
                ...
                try:
                    ...
                    suffix = (
                        Path(file.split("?")[0]).suffix.lower()
                        or f".{guessed_ext or 'tmp'}"
                    )
                    if not suffix or suffix not in cls._registry:
                        raise DocumentLoaderError(
                            f"Unsupported or undetectable file type for URL: {file}"
                        )
...
def load_document(
    file: tp.Union[str, Path, UploadFile], extract_text_only: bool = True
) -> tp.Generator[str, None, None]:
    """
