...
class PubSub(tp.Generic[T]):
    ...
    async def pub(self, channel: str, event: Event[T]) -> None:
        """Publish a strongly typed Event[T]"""
        await _db.publish(channel, event.model_dump_json())  # type: ignore

    async def sub(self, channel: str) -> tp.AsyncGenerator[Event[T], None]:
        ...
        while True:
            ...
            if msg and msg["type"] == "message":
                try:
                    ...
                    event = Event[self._model].model_validate_json(raw)  # type: ignore
                    yield event
                except Exception:
                    continue
