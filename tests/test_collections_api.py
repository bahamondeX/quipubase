...
def test_collection_actions(client, cleanup):
    ...
    response = client.post(f"/v1/collections/{collection_id}/action", json=update_payload)
    assert response.status_code == status.HTTP_200_OK
    ...
    response = client.post(f"/v1/collections/{collection_id}/action", json=delete_payload)
    assert response.status_code == status.HTTP_200_OK
