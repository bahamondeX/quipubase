...
from quipubase.api.audio.transcriptions.typedefs import (
    TranscriptionCreateParams, TranscriptionResponse,
...
class TestTranscriptionCreateParams:
    ...
    def test_default_values(self):
        ...
        params = TranscriptionCreateParams(file="test.mp3", model="whisper-1")
    ...
    def test_temperature_validation(self):
        ...
        # Valid temperature
        params = TranscriptionCreateParams(
            file="test.mp3", model="whisper-1", temperature=0.8
        )
        ...
        with pytest.raises(ValueError):
            TranscriptionCreateParams(
                file="test.mp3", model="whisper-1", temperature=1.5  # Too high
            )
...
class TestTranscriptionService:
    ...
    async def test_create_transcription(self, service):
        ...
        params = TranscriptionCreateParams(
            file="test.wav", model="whisper-1", response_format="json"
        )
...
class TestTranscriptionsAPI:
    ...
    async def test_transcriptions_endpoint_json(self, client, mock_audio_file):
        ...
        with patch(
            "quipubase.api.audio.transcriptions.router.transcription_service"
        ) as mock_service:
    ...
    async def test_transcriptions_endpoint_text(self, client, mock_audio_file):
        ...
        with patch(
            ...
            mock_service.create_transcription = AsyncMock(return_value="Hello world")
    ...
    @pytest.mark.asyncio
    async def test_transcriptions_endpoint_with_optional_params(
        self, client, mock_audio_file
    ):
    ...
    @pytest.mark.asyncio
    async def test_transcriptions_endpoint_error_handling(
        self, client, mock_audio_file
    ):
    ...
    def test_transcriptions_endpoint_invalid_model(self, client, mock_audio_file):
        ...
        response = client.post(
            "/audio/transcriptions",
            files={"file": ("test.mp3", mock_audio_file, "audio/mp3")},
            data={"model": "invalid-model", "response_format": "json"},
        )
...
class TestOpenAIClientCompatibility:
    ...
    async def test_response_format_compatibility(self, client, mock_audio_file):
        ...
        ) in formats_and_expected_content_types:
            with patch(
                "quipubase.api.audio.transcriptions.router.transcription_service"
            ) as mock_service:
...
class TestActualOpenAIClient:
    ...
    def test_with_openai_client(self):
        ...
        """
        pytest.skip(
            "Integration test - requires running server and OpenAI client setup"
        )
