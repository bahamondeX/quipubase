...
from quipubase.vector.typedefs import (DeleteText, Embedding, EmbedText,
                                       QueryText)
...
def test_upsert_query_delete_flow(client: TestClient):
    ...
    for match in query_data["data"]:
        ...
        assert (
            match["score"] > 0 and match["score"] <= 1
    ...
    response = client.put(
        f"/v1/vector/{namespace}",
        json={"content": ["Hello testing"], "model": "mini-scope", "top_k": 2},
    )
    assert response.status_code == 200
    query_data = response.json()
...
def test_vector_store_service():
    ...
    assert (
        query_response["data"][0].content == text
    )  # Access as attribute, not subscriptable
...
def test_multiple_namespaces(client: TestClient):
    ...
    response1 = client.put(
        f"/v1/vector/{namespace1}",
        json={
            "content": ["Test text for multiple namespaces"],
            "model": "mini-scope",
            "top_k": 1,
        },
    )
