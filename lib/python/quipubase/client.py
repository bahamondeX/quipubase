...
from .typedefs import (Collection, CollectionMetadataType, CollectionType,
                       JsonSchema, Request)
from .typedefs import Response as QResponse
...
class QuipuBase(tp.Generic[T], LazyProxy[AsyncClient]):
    ...
    async def fetch(
        self,
        endpoint: str,
        method: tp.Literal["GET", "POST", "PUT", "DELETE"],
    ...
    async def pub(self, col_id: str, request: Request[T]) -> QResponse[T]:
        ...
        action_request: dict[str, tp.Any] = {
            ...
            "data": (
                request.data.model_dump(exclude_unset=True, exclude_none=True)
    ...
    async def sub(self, col_id: str):
        ...
        client = AsyncClient(base_url=self.base_url, timeout=None)  # No timeout
        while True:
            try:
